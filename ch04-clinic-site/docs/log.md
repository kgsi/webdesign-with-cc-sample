...
- reserve は Figma の `reserve` フレーム（2:66）と照合し一致：上下余白 80px（コードは `py-20`）、電話番号 28px（`text-h2`）、右カラムは「お電話でのご予約」「03-1234-5678」「受付時間 9:00〜18:00（平日）」の3行。コードは変えていない。指示の範囲外として、左カラムのボタン上余白が Figma 16px、コード 24px（`mt-lg`）である差も報告された
