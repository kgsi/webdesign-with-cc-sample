...
- reserve は Figma の `reserve` フレーム（2:66）と照合し一致：上下余白 80px（コードは `py-20`）、電話番号 28px（`text-h2`）、右カラムは「お電話でのご予約」「03-1234-5678」「受付時間 9:00〜18:00（平日）」の3行。コードは変えていない。指示の範囲外として、左カラムのボタン上余白が Figma 16px、コード 24px（`mt-md`）である差も報告された
...
- 残り：header の受付時間の表記「受付」と reserve の「受付時間」が揃っていない。用語の統一は 4-4 のルール化で扱う。reserve 左カラムのボタン上余白（Figma 16px、コード 24px）も 4-4 へ

## 4-4 v5-rules（ズレの振り分けとルール化）

### 前提

v1 から v4 までで見つかり、周の中で扱わなかったズレ7件を、修正指示・Figmaへ戻す・制作ルールの3つに振り分けた（2026-09-21）。

| # | ズレ | 層 | 行き先 | 処置 |
|---|---|---|---|---|
| 1 | message の H2「患者さんに寄り添う医療を、これからも」の「も」だけが2行目に落ちる。Figma でも同じ | 構造 | Figmaへ戻す | Figma の `message` フレームの H2（2:33）を読点の後で改行した2行に直し、`wireframe/03-message.png` を書き出し直した。構造記述にも「2行」と書いた。コードは `<br>` |
| 2 | mint の用途が `docs/tone.md` にだけあり、CLAUDE.md の Card は pale のまま。次の生成で pale に戻るおそれ | ルール | 制作ルール | CLAUDE.md の色の用途と Card の定義を mint に直し、`docs/rules.md` と Figma ルールページの Card コンポーネント（4:98、アイコン 4:99）の塗りも color/mint に付け替えた |
| 3 | muted（#6F8697）が白地で 3.8:1、bg 地で 3.6:1 と要件の 4.5:1 未満 | トーン | 制作ルール | 値を #5C7384（白地 4.9:1、bg 地 4.7:1）へ変更。Figma の変数 color/muted、CLAUDE.md、`docs/tone.md`、`docs/rules.md`、CSS 変数を同時に更新した。使う箇所は変えない |
| 4 | reserve の縦余白 80px がトークン（space/*）にない | 細部 | 制作ルール（記録） | 1箇所だけなので変数を増やさない。CLAUDE.md の「Figma と意図的に違えている点」に理由を残す |
| 5 | Card の本文がワイヤーフレームでは薄い灰色、コードでは ink | 細部 | 制作ルール（記録） | 意図的な不一致として CLAUDE.md に残す。「本文サイズの文字に muted を使わない」と書いた |
| 6 | header は「受付 9:00〜18:00（平日）」、reserve は「受付時間 9:00〜18:00（平日）」で表記が揃わない | 構造 | 制作ルール＋修正指示 | CLAUDE.md に「用語」を足し、header の表記を直す。構造記述も直した |
| 7 | reserve 左カラムのボタン上余白が Figma 16px、コード 24px | 細部 | 修正指示 | `mt-md` を `mt-sm` に直す。Figma は変えない |

Figma へ戻したのは1件（#1）、変数とコンポーネントを書き戻したのは2件（#2、#3）、記録だけ残したのは2件（#4、#5）、コードを直すのは4件（#1、#3、#6、#7）。Figma の変更は `use_figma` で行い、ノード ID と変数 ID は上の表のとおり。

### プロンプト（Claude Codeへ、ch04-clinic-site で新規セッション）

```
CLAUDE.md を更新しました（color/muted の値、mint と pale の用途、Card の定義、「用語」と「Figma と意図的に違えている点」の追加）。更新後の CLAUDE.md に合わせて index.html を直してください。変更は次の4点だけです。

1. <style> の --color-muted を CLAUDE.md の値（#5c7384）に変えてください。クラス名や使っている箇所は変えません。
2. header の受付時間の表記を CLAUDE.md の「用語」に合わせ、「受付時間 9:00〜18:00（平日）」にしてください。
3. message セクションの H2「患者さんに寄り添う医療を、これからも」を、Figma の「message」フレーム（wireframe/03-message.png、更新済み）と同じく読点の直後で改行してください。hero の H1 と同じく <br> を入れます。
4. reserve セクションの左カラムにある「診療予約はこちら」ボタンの上余白を、Figma の reserve フレームと同じ 16px（mt-sm）にしてください。現状は mt-md（24px）です。

制約
- 変更するのは index.html の上記4箇所だけ。他のセクション、tailwind.config、docs には触れない
- CLAUDE.md の「完了前の確認」を実行し、色クラスが9色に収まっていることを確認する
- 変更したファイル、差分の要点、判断に迷った点を報告する
```
